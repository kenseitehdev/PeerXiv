...
### Publish an invite-only alpha through ngrok

After installing and authenticating ngrok, use the assigned development domain
to create a private local configuration and start the HTTPS/WebSocket tunnel:
...
The first command writes gitignored random secrets to `.env.ngrok`; it refuses
to overwrite an existing configuration. The second command retains the alpha
database and manuscripts under `instance/`, binds Gunicorn only to loopback,
runs migrations, and stops Gunicorn when the tunnel closes. Keep the Mac awake
and back up `instance/`. This zero-cost profile is for trusted invitees, not
open public submissions. See [DEPLOYMENT.md](DEPLOYMENT.md) for its security and
